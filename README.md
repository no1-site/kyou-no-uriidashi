...
Windowsで `scripts/run-update-dry-run.cmd` をダブルクリックします。現在のPCに保存済みの楽天・Yahoo!の暗号化認証情報を使い、楽天 → Yahoo! → Keepa（保存済みキーがある場合のみ）の実API取得と、日次更新と同じ公開前検証までを実行します。楽天・Yahoo!の認証が未設定なら失敗として終了します。Keepaのキーの新規設定・契約は行いません。設定済みのKeepaを使う場合は通常のAPIトークンを消費します。
...
終了時は次の7項目だけを日本語で表示します。ウィンドウを閉じるには何かキーを押してください。
